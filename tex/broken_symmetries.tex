...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
      ...
      \draw[->] (0,0) -- (0,2) node[above] {$\s(\omega)$};
      \draw[blue] plot[samples=200,domain=0:2] function {(1 + x**2 )* 2.7**(-x**2)};
\end{tikzpicture}
\end{figure}

where $\pdv{\s}{\omega} < 0 \implies \frac{1}{1 - \pdv{\s}{\omega}} < 1$. (Karl) In general, we say that if $Z_k \neq 0$, the system is a Fermi liquid. \\ 
